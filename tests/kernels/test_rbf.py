...
class TestRBFKernelInit:
    def test_isotropic_scalar(self) -> None:
    ...
    def test_anisotropic_list(self) -> None:
    ...
    def test_negative_length_scale_raises(self) -> None:
        with pytest.raises(ValidationError):
...
class TestRBFGetSetParams:
    ...
    def test_isotropic_wrong_num_params_raises(self) -> None:
...
class TestRBFComputeIsotropic:
    ...
    def test_1d_input_accepted_via_compute(self) -> None:
...
class TestRBFComputeAnisotropic:
    ...
    def test_different_from_isotropic(self) -> None:
...
class TestRBFComputeDiag:
    def test_isotropic_diag_is_ones(self) -> None:
        k = RBFKernel(length_scale=1.0)
        x = _make_x(10, 3)
...
class TestRBFGradientIsotropic:
    ...
    def test_gradient_output_is_tuple(self) -> None:
        x = _make_x(5, 2)
        grads = self.kernel._gradient(x, x)
        assert isinstance(grads, tuple)

    def test_gradient_shape_isotropic(self) -> None:
    ...
    @pytest.mark.slow
    def test_gradient_matches_finite_difference_isotropic(self) -> None:
...
class TestRBFGradientAnisotropic:
    ...
    def test_gradient_shape_anisotropic(self) -> None:
    ...
    @pytest.mark.slow
    def test_gradient_matches_finite_difference_anisotropic(self) -> None:
...
class TestRBFComputeWithGradient:
    def test_isotropic_K_matches_compute(self) -> None:
        k = RBFKernel(length_scale=1.0)
        x = _make_x(5, 2)
    ...
    def test_anisotropic_K_matches_compute(self) -> None:
        k = RBFKernel(length_scale=[1.0, 2.0], isotropic=False)
        x = _make_x(5, 2)
        K_direct = k._compute(x, x)
        K_with_grad, _ = k._compute_with_gradient(x, x)
    ...
    @pytest.mark.slow
    def test_isotropic_grad_matches_gradient_method(self) -> None:
    ...
    @pytest.mark.slow
    def test_anisotropic_grad_matches_gradient_method(self) -> None:
        k = RBFKernel(length_scale=[1.0, 2.0], isotropic=False)
        x = _make_x(4, 2)
        _, (grad_cwg,) = k._compute_with_gradient(x, x)
        (grad_sep,) = k._gradient(x, x)
...
class TestRBFCall:
    def test_single_arg_computes_self_kernel(self) -> None:
        k = RBFKernel(length_scale=1.0)
        x = _make_x(5, 2)
        K_call = k(x)
    ...
    def test_two_args_computes_cross_kernel(self) -> None:
        k = RBFKernel(length_scale=1.0)
        x1 = _make_x(5, 2)
        x2 = _make_x(7, 2)
        K_call = k(x1, x2)
...
class TestRBFToStr:
    ...
    def test_string_contains_variable_names(self) -> None:
        k = RBFKernel(length_scale=1.0)
