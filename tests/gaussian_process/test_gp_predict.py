...
class TestGPPredictMeanOnly:
    ...
    def test_returns_array_not_tuple(self) -> None:
        gp, _, _ = _fitted_gp()
    ...
    def test_predictions_close_to_training_at_training_points(self) -> None:
...
class TestGPPredictWithStd:
    ...
    def test_mean_and_std_shapes(self) -> None:
        gp, _, _ = _fitted_gp()
        x_test = np.linspace(0, 2 * np.pi, 10).reshape(-1, 1)
    ...
    def test_std_smaller_at_training_points(self) -> None:
...
class TestGPPredictWithCov:
    ...
    def test_covariance_diagonal_nonnegative(self) -> None:
        gp, _, _ = _fitted_gp()
        x_test = np.linspace(0, 2 * np.pi, 5).reshape(-1, 1)
        _, y_cov = gp.predict(x_test, return_cov=True)
...
class TestGPPredictStdAndCov:
    ...
    def test_std_equals_sqrt_of_cov_diag(self) -> None:
        gp, _, _ = _fitted_gp()
...
class TestGPPredict2D:
    def test_2d_predict_output_shape(self) -> None:
