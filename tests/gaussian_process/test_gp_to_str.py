...
class TestGPToStrUnfitted:
    def test_unfitted_returns_empty_string(self) -> None:
        gp = GaussianProcess(RBFKernel(length_scale=1.0))
...
class TestGPToStr1D:
    ...
    def test_string_contains_variable_name(self) -> None:
        gp = _fitted_gp_1d()
        result = gp.to_str("theta")
        assert "theta" in result

    def test_string_contains_exp(self) -> None:
        """RBF kernel string should contain 'exp'."""
        gp = _fitted_gp_1d()
        result = gp.to_str("x")
        assert "exp(" in result

    def test_single_string_accepted(self) -> None:
        """A plain string (not a list) should be accepted for 1D GPs."""
        gp = _fitted_gp_1d()
        result = gp.to_str("x")
        assert isinstance(result, str)

    def test_list_with_one_element_accepted(self) -> None:
        gp = _fitted_gp_1d()
        result = gp.to_str(["x"])
        assert isinstance(result, str)
...
class TestGPToStr2D:
    ...
    def test_string_contains_both_variable_names(self) -> None:
        gp = _fitted_gp_2d()
...
class TestGPToStrValidation:
    ...
    def test_too_few_variables_raises(self) -> None:
...
class TestGPToStrStandardization:
    def test_standardized_string_contains_x_mean(self) -> None:
        """Standardized GPs embed x_mean into the string expression."""
        gp = _fitted_gp_1d()
        result = gp.to_str("x")
        assert len(result) > 20

    def test_non_standardized_string_does_not_embed_x_mean(self) -> None:
