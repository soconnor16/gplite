...
def empty_pool_learner() -> ActiveLearner:
    ...
    y = np.sin(x).ravel()
    learner = ActiveLearner(
        kernel=RBFKernel(length_scale=1.0), x_full=x, y_full=y
    )
    learner.gp.fit(learner.x_train, learner.y_train)
...
class TestRandomSelection:
    def test_returns_array(self, fitted_learner) -> None:
        indices = random_selection(fitted_learner, n_points=1)
        assert isinstance(indices, np.ndarray)

    def test_returns_correct_count(self, fitted_learner) -> None:
        indices = random_selection(fitted_learner, n_points=3)
        assert len(indices) == 3

    def test_indices_within_remaining(self, fitted_learner) -> None:
        indices = random_selection(fitted_learner, n_points=5)
        for idx in indices:
            assert idx in fitted_learner.remaining_indices

    def test_no_duplicate_indices(self, fitted_learner) -> None:
        indices = random_selection(fitted_learner, n_points=5)
        assert len(set(indices)) == len(indices)

    def test_empty_pool_returns_empty(self, empty_pool_learner) -> None:
        indices = random_selection(empty_pool_learner, n_points=1)
        assert len(indices) == 0

    def test_request_more_than_pool_returns_all(self, fitted_learner) -> None:
    ...
    def test_reproducible_with_seed(self, fitted_learner) -> None:
...
class TestMaxAbsoluteError:
    ...
    def test_selected_point_has_highest_error(self, fitted_learner) -> None:
...
class TestExpectedImprovementMin:
    ...
    def test_ei_max_and_min_can_differ(self, fitted_learner) -> None:
