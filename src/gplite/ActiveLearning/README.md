...
3. Acquire: Selects and labels highest-scoring point(s) and moves them from
...
variance (σ²) is highest. This is a pure exploration strategy, focusing on
...
* **Expected Improvement**("`ei_min`"/"`ei_max`"): Balances exploration
(high uncertainty) and exploitation (high predicted mean for maximization,
or low predicted mean for minimization). Ideal for optimization tasks
...
Evaluates the remaining pool using a specific scoring function and returns the
indices of the top candidates. This is a lower-level method useful if you want
to write a completely manual training loop.
