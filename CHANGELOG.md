...
- **Custom Kernel Bounds** Users may now pass custom Kernel bounds during its
initialization, which will be passed along during hyperparameter optimization.

### Fixed

- **Normalization vs Standardization** Changed incorrect uses of "normalization"
to "standardization" where appropriate.

### Changed

- **Kernel Bounds** Kernel bounds are now returned as a dictionary rather than a
flat list of tuples, which should make their parsing and visualization simpler.

### Other Changes

- **Documentation and Formatting** Documentation for all files has been
standardized, reformatted (following the google docstring style), or reworded
for readability.

### Breaking Changes

- **Gaussian Process Initialization** The "normalize_inputs" boolean argument
has been changed to "standardize_inputs"
...
- **Formatting and Documentation** Ensured files were formatted consistently
...
- Gaussian Process regression with automatic hyperparameter optimization via
...
- String export of fitted GP expressions for integration with external tools
(e.g., OpenMM)
